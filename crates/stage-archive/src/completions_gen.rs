...
/// Unbind the transient template vars bound for a single mode-A generation
/// call (`Shell` and `ArtifactPath`) so they never outlive it. Setting them
/// to empty (rather than leaving the last shell / host-binary path bound)
/// keeps `{{ .Shell }}` / `{{ .ArtifactPath }}` rendering empty everywhere
/// outside generation.
///
/// `ArtifactPath` matters because generation runs BEFORE the per-target
/// archive loop, and the loop only re-binds `ArtifactPath` AFTER the archive
/// write — so without this clear the stale host-binary path would leak into
/// the archive `name_template`, `wrap_in_directory`, `files:` src/glob, and
...
fn clear_generate_vars(ctx: &mut Context) {
...
fn run_generate(
    ...
    // Bind the mode-A template surface, then render the command. `Shell` and
    // `ArtifactPath` are bound only for the duration of THIS render and
    // cleared immediately after (see `clear_generate_vars`) so neither
    // outlives the call: a leftover `Shell` would bleed into the man-page
    // `generate:` command (shell=None), and a leftover `ArtifactPath` (the
    // host-binary path) would bleed into every subsequent archive-name /
    // wrap_in_directory / files / templated_files render for the crate.
    let tvars = ctx.template_vars_mut();
    ...
    // Clear the transient vars before any early return so they cannot leak
    // even when rendering or the command itself fails.
    clear_generate_vars(ctx);
    let cmd = cmd?;
