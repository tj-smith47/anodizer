...
pub use helpers::{
    VALID_SIGN_ARTIFACT_FILTERS, sign_filters_can_overlap, sign_outputs_are_one_file,
};
...
/// Publishers that consume the sign stage's detached signature / certificate
/// artifacts in publish-only mode.
///
/// `github-release` and `blob` upload them via `release_uploadable_kinds`;
/// `artifactory` and `uploads` upload them when an entry sets
/// `signature: true`. The sign loops self-skip only when EVERY one of these is
/// deselected (see [`signs_fully_deselected`]), so a selected signature
/// consumer can never be starved of the sidecars it was configured to publish.
/// `binary_signs:` output hangs off the same list: a binary signature is an
/// ordinary release asset, so the publishers that read it are the publishers
/// that read a `signs:` sidecar. This is the single source of truth shared by the
...
/// True when no selected publisher consumes signature output, so both sign
/// loops have no downstream sink and can be skipped.
///
...
impl Stage for SignStage {
    ...
    fn run(&self, ctx: &mut Context) -> Result<()> {
        ...
        // ----------------------------------------------------------------
        //
        // Consumer-selection gate. `sign` is a PREP stage, not a publisher, so
        // it has no `--publishers` identity of its own. Both slices produce
        // detached signatures and certificates that upload as release assets,
        // and the publishers that read them are `signs_consumers()`. When
        // EVERY one of those is deselected (e.g. `--publishers npm`), no
        // selected surface will ever read the output, so producing it would
        // demand cosign/GPG material a publisher-scoped runner does not carry
        // for no downstream effect. The consumer set is the single source of
        // truth in `signs_consumers()`, so the runtime gate and the preflight
        // gate can never diverge. An EMPTY allowlist deselects nothing, so the
        // main release job still signs.
        if signs_fully_deselected(ctx) {
            if !ctx.config.signs.is_empty() || !ctx.config.binary_signs.is_empty() {
                log.status(&format!(
        ...
        // ----------------------------------------------------------------
        //
        // A binary that is not on disk — the publish-only case, where the
        // preserved dist carries the release assets but not the raw cargo
        // output — drops out inside `process_sign_configs`, which records the
        // config's skip instead of handing the signer a missing path.
        let binary_sign_configs = ctx.config.binary_signs.clone();
        process_sign_configs(
            &binary_sign_configs,
            ctx,
    ...
    fn run(&self, ctx: &mut Context) -> Result<()> {
        ...
        if let Some(docker_signs) = ctx.config.docker_signs.clone() {
            ...
            for docker_sign_cfg in &docker_signs {
                ...
                let image_paths: Vec<docker_render::ImageEntry> = docker_artifacts
                    .into_iter()
                    .filter(|a| {
                ...
                // Every image's argv and env, rendered before anything is
                // spawned; `None` is the harness skip for keyless cosign.
                let Some(per_image) = render_image_signs(
                    ctx,
                    &log,
                    docker_sign_cfg,
                    sign_id,
                ...
                for ((image_path, metadata), image) in image_paths.iter().zip(&per_image) {
                    let RenderedImageSign {
                        signed_ref,
                        argv: fully_resolved,
                        env: docker_rendered_env,
                    } = image;
