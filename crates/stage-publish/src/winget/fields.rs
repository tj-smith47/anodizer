        ...
        _ => {
            if repo_owner.is_empty() {
                return Err(anodizer_core::pipe_skip::entry_skip(
                    "publisher is required but not configured, and repo owner is \
                     also empty. Set `publish.winget.publisher` in your config.",
                ));
            }
            log.warn(&format!(
    ...
    let short_desc_raw = winget_cfg
        ...
        .ok_or_else(|| {
            anodizer_core::pipe_skip::entry_skip(
                "short_description is required but not configured. Set \
                 `publish.winget.short_description`, or a fallback via \
                 `publish.winget.description` or top-level `metadata.description`.",
            )
        })?;
...
pub(crate) fn resolve_winget_license<'a>(
    ...
    winget_cfg
        ...
        .ok_or_else(|| {
            anodizer_core::pipe_skip::entry_skip(
                "license is required but not configured. Set \
                 `publish.winget.license` in your config.",
            )
        })
}
