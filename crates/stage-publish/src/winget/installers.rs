    ...
    if artifact_kind == anodizer_core::artifact::ArtifactKind::Installer {
        ...
        if binary_count > 0 && zip_count > 0 {
            return Err(anodizer_core::pipe_skip::entry_skip(
                "found archives with multiple formats (.exe and .zip); use either \
                 portable binaries or zip archives, not both",
            ));
        }
    }
    ...
    for (arch, count) in &arch_counts {
        if *count > 1 {
            return Err(anodizer_core::pipe_skip::entry_skip(format!(
                "found multiple archives for the same platform ({arch})"
            )));
        }
    }
