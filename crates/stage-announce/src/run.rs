...
mod summary_tests {
    ...
    /// A publish stage that aborts on a pre-dispatch guard (here: an
    /// `--allow-nondeterministic` name matching no emitted artifact) must
    /// surface as the `aborted before dispatch` placeholder row. This pins
    /// two things at once: the disposition derivation in `emit_summary` AND
    /// that `set_publish_attempted()` fires BEFORE the pre-dispatch guards —
    /// moving it below them (or deleting it) regresses the row to the
    /// mislabeled "publish stages did not run".
    #[test]
    fn emit_summary_reports_abort_before_dispatch() {
        ...
        ctx.options
            ...
            .push(("no-such-artifact".to_string(), "typo".to_string()));
        ...
        let err = anodizer_stage_publish::PublishStage
            .run(&mut ctx)
            .expect_err("an unmatched allowlist name must abort before dispatch");
        assert!(
            err.to_string()
                .contains("did not match any emitted artifact"),
            "abort must come from the allowlist guard, not some other failure: {err:#}"
        );
        assert!(
