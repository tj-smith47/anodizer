...
pub struct Context {
    ...
    /// The builder-owned temp directory behind a test context's `dist`,
    /// removed when the context drops.
    ///
    /// Set only by `TestContextBuilder` when the test named no explicit dist;
    /// an explicit `dist` leaves this `None` and stays the caller's own
    /// directory to manage.
    ///
    /// Gated behind the `test-helpers` Cargo feature — production
    /// binaries do not carry the field at all.
    #[cfg(feature = "test-helpers")]
    pub private_dist: Option<tempfile::TempDir>,
