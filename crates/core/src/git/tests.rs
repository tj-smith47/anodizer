...
pub(super) struct BufferWriter(std::sync::Arc<std::sync::Mutex<Vec<u8>>>);
...
/// `make_writer` returns a cheap guard that writes into the same buffer as
/// every other clone, which is what the `MakeWriter` contract asks for.
...
pub(super) struct BufferWriterGuard<'a>(std::sync::MutexGuard<'a, Vec<u8>>);
...
/// Run `body` with a thread-local tracing subscriber and return everything it
/// emitted at `warn` or above.
pub(super) fn capture_tracing_warnings<F: FnOnce()>(body: F) -> String {
