...
- [ ] 2026-04-27 dogfood (WAVE 6): `.anodizer.yaml` snapshot dry-run emits 75 `Warning: artifact 'anodizer_<ver>_linux_amd64.tar.gz' already registered` lines because every per-crate archive resolves to the same filename. The default `archive.name_template` is the GoReleaser-canonical `"{{ .ProjectName }}_{{ .Version }}_{{ .Os }}_{{ .Arch }}"`, which omits the per-crate `name` axis; with 26 crates inheriting `defaults.archives` (`format: tar.gz`, `format_overrides: [{os: windows, formats: [zip]}]`), all 26 produce the same `anodizer_<ver>_<os>_<arch>` filename. Same warning count (75) on master before the WAVE 6 YAML migration — defaults inheritance neither introduced nor masked the bug; the duplicate-filename collision sits in the archive name-template defaults. Fix options: (a) make the default `archive.name_template` include `{{ .CrateName }}` in monorepo / multi-crate configs; (b) document a recommended per-crate override in the dogfood README; (c) special-case "single-binary monorepo" so non-binary crates don't produce archives. Tracked here for the Session C behavior pass; the WAVE 6 schema migration is not the right scope for the artifact-naming fix.
