...
### WAVE 6 deferred — 2026-04-27 (1 WARN)
...
- [x] 2026-04-27 code-review (WAVE 1.5): consider extracting parse_env_entries / split_env_entry / render_env_entries to crates/core/src/env.rs once WAVE 2 adds defaults env merging — currently 3 helpers + 7 tests sit in 10K-line config.rs. — resolved 2026-04-28. Helpers + 7 unit tests lifted to `crates/core/src/env.rs` (companion to the existing `env_expand`). `config.rs` keeps `pub use crate::env::{...}` so `anodizer_core::config::*` import paths stay stable for stages and publishers. Two additional regression tests added for `render_env_entries` (passthrough + render-error propagation).

### WAVE 2 deferred — 2026-04-27 (1 SUGGEST)

- [x] 2026-04-27 code-review (WAVE 2): `apply_defaults` does not yet fold top-level `Config` fields (`source`, `upx`, `sign`, `binary_signs`, `docker_signs`, `notarize`, `sbom`, `makeselves`, `srpms`) — these live on `Config`, not `CrateConfig`, but path-mirror in `Defaults`. Decide in WAVE 3 or 5 whether to fold pre-pipeline (so the resolved `Config.<topfield>` reflects defaults) or leave `Defaults.<topfield>` as inert (per-crate `CrateConfig.<field>` is the only consumer). See `crates/core/src/defaults_merge.rs::apply_defaults` rustdoc. — resolved 2026-04-28. Folded pre-pipeline. New `apply_top_level_defaults(config, defaults)` in `crates/core/src/defaults_merge.rs` runs after the per-crate fold. Single-struct `Option<T>` slots (`source`, `notarize`, `srpms`) deep-merge through the existing `deep_merge_option` helper; `Vec<T>` slots (`upx`, `signs`, `binary_signs`, `sboms`, `makeselfs`) and the `Option<Vec<T>>` `docker_signs` slot are filled from the single defaults entry only when empty (user-set top-level wins, defaults are inert). Six new regression tests pin: source fill + user-overrides, upx fill + user-vec wins, signs/binary_signs/docker_signs/sboms/makeselfs/srpms fill, and notarize deep-merge.
