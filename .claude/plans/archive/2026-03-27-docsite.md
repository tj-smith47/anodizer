> **STATUS (archived 2026-05-06):** All 72 outlined items delivered and
> extended. Zola site live at `docs/site/` with publish (24 pages),
> announce (15 pages), packages (17), advanced (8), ci (5); xtask
> `gen_docs.rs` (~20K lines) drives schema-introspected reference; Pages
> deploy via `.github/workflows/docs.yml`. Canonical tracker:
> `parity-session-index.md` (Phase 1) + follow-on `2026-03-27-docsite-phase-b.md`.
> Per-checkbox reconciliation skipped — not load-bearing for active work.

# Anodizer Documentation Site — Implementation Plan
...
| File | Responsibility |
|------|---------------|
| `Cargo.toml` | Crate manifest — depends on `anodizer` CLI crate (as lib), `anodizer-core`, and `tera` |
...
[extra]
github_url = "https://github.com/tj-smith47/anodizer"
```
...
<body>
  <nav class="top-nav">
    <div class="nav-inner">
      <a href="{{ config.base_url }}" class="nav-brand">anodizer</a>
      <div class="nav-links">
  ...
  <footer class="site-footer">
    <div class="footer-inner">
      <p>&copy; 2026 Anodizer Contributors. MIT License.</p>
...
{% block title %}Anodizer — Release engineering for Rust, simplified.{% endblock %}
...
<section class="hero">
  <div class="hero-inner">
    ...
    <div class="terminal-demo">
      ...
      <pre class="terminal-body"><span class="terminal-prompt">$</span> <span class="terminal-cmd">cargo install anodizer</span>
<span class="terminal-prompt">$</span> <span class="terminal-cmd">anodizer init</span>          <span class="terminal-comment"># generates .anodizer.yaml from Cargo.toml</span>
<span class="terminal-prompt">$</span> <span class="terminal-cmd">anodizer release</span>       <span class="terminal-comment"># build → archive → checksum → release → publish</span></pre>
    </div>
  </div>
</section>
...
<section class="features">
  <div class="features-grid">
    ...
    <div class="feature-card">
      <h3>Single Binary</h3>
      <p><code>cargo install anodizer</code>. No runtime dependencies. Fast startup.</p>
    </div>
  </div>
</section>
{% endblock %}
...
After building both `x86_64-apple-darwin` and `aarch64-apple-darwin` targets, anodizer will run `lipo -create -output` to produce a universal binary. The universal binary is registered as its own artifact for archiving and release.
```
...
After the build stage, anodizer will run `upx` on matching binaries with the configured compression flags. If `upx` is not installed, the step is skipped with a warning (unless `required: true`).
```
...
{% coming_soon() %}
Chocolatey publishing is planned for a future release. Anodizer will generate `.nuspec` manifests and `chocolateyInstall.ps1` scripts, then publish via `choco push`.
{% end %}
```
...
{% coming_soon() %}
Winget publishing is planned for a future release. Anodizer will generate YAML manifests and submit PRs to the winget-pkgs repository via the GitHub API.
{% end %}
```
...
{% coming_soon() %}
AUR publishing is planned for a future release. Anodizer will generate PKGBUILD files and publish to the AUR via `makepkg` and SSH.
{% end %}
```
...
{% coming_soon() %}
Krew publishing is planned for a future release. Anodizer will generate kubectl plugin manifest YAML with download URIs and SHA-256 checksums.
{% end %}
```
...
Combine with `anodizer tag` to automatically create tags from commit messages:
    ...
    steps:
      - uses: actions/checkout@v4
        with:
          fetch-depth: 0
      - uses: dtolnay/rust-toolchain@stable
      - run: cargo install anodizer cargo-zigbuild
      - run: anodizer release
        env:
          GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
```
...
- uses: actions/cache@v4
  with:
    path: ~/.cargo/bin/anodizer
    key: anodizer-${{ runner.os }}
```
```
...
Anodizer supports Cargo workspaces with independent release cadences per crate.
...
anodizer release --crate core-lib

# Release just the CLI
anodizer release --crate cli-tool
```

### Dependency ordering

Use `depends_on` to ensure crates are released in the right order. Anodizer performs topological sorting — if `cli-tool` depends on `core-lib`, `core-lib` is always released first.
...
{% coming_soon() %}
Config includes are planned for a future release. This feature will allow splitting your `.anodizer.yaml` across multiple files for better organization and reuse.
{% end %}
...
{% coming_soon() %}
Reproducible build support is planned for a future release. When enabled, anodizer will set `SOURCE_DATE_EPOCH`, strip non-deterministic metadata from archives, and pass `--remap-path-prefix` to rustc.
{% end %}
...
> This page will be auto-generated from the CLI source code. For now, see `anodizer --help` for the full command reference.

## Commands

| Command | Description |
|---------|-------------|
| `anodizer release` | Run the full release pipeline |
| `anodizer build` | Build binaries only |
| `anodizer check` | Validate configuration |
| `anodizer init` | Generate starter config |
| `anodizer changelog` | Generate changelog only |
| `anodizer tag` | Auto-tag based on commit directives |
| `anodizer completion` | Generate shell completions |
| `anodizer healthcheck` | Check tool availability |

Run `anodizer <command> --help` for detailed flag information.
```
...
> This page will be auto-generated from the config types. For now, see the [full example config](https://github.com/tj-smith47/anodizer/blob/main/.anodizer.yaml) for a comprehensive reference.

Anodizer uses `.anodizer.yaml` (or `.anodizer.toml`) in your project root. Both formats use the same schema.
...
5. Replace the cargo-dist GitHub Actions workflow with an anodizer workflow
...
- Tests in `main.rs` use `pub use anodizer_cli::*;` to ensure `use super::*` still works
...
```rust
pub use anodizer_cli::{Cli, Commands, num_cpus};
...
```bash
cargo build --package anodizer && cargo test --package anodizer
```
...
In root `Cargo.toml`, add `tera` to `[workspace.dependencies]` (it's currently only a direct dep of `anodizer-core`):
...
[dependencies]
anodizer = { path = "../cli" }
anodizer-core = { path = "../core" }
clap = { version = "4", features = ["derive"] }
tera.workspace = true
```

Note: The CLI crate's package name is `anodizer` (not `anodizer-cli`). The `[lib]` target is named `anodizer_cli`, so in Rust code we `use anodizer_cli::build_cli`. Cargo resolves the dependency by package name (`anodizer`) and the lib name controls the Rust import path.
...
fn generate_config_reference(tera: &Tera) -> Result<String, String> {
    ...
    // This uses the known field names from anodizer_core::Config.
