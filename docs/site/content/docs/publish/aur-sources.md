...
Not applicable as a config flag — AUR source publishing is always a push that overwrites the previous PKGBUILD. The Manager group rollback reverts the commit via `git revert` + push.
