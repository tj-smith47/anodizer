...
| `tag_template` | string | — | Git tag template used to tag and identify releases (supports templates). Overrides `defaults.crates.tag_template`. When both are unset the crate's tag family is the `<name>-v{{ Version }}` convention — read it through `tag_family_template()` rather than touching this field directly. |
