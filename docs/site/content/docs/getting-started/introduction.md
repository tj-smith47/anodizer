...
> **Why `homebrew_casks`, not `publish.homebrew`?** The cask is the canonical channel for pre-compiled binaries. The older `publish.homebrew` *Formula* path still parses for back-compat but emits a deprecation warning at config-load. New configs should use `homebrew_casks`. See [Homebrew](@/docs/publish/homebrew.md) for both forms and the [migration guide](@/migration/goreleaser.md) if you're coming from GoReleaser.
