...
| Key | Status | Notes |
|---|---|---|
| `anodize check determinism --runs=N` harness | ✅ Verified | [anodizer `release.yml`](https://github.com/tj-smith47/anodizer/blob/v0.12.3/.github/workflows/release.yml) (`determinism-check:` matrix runs the harness on every tag push; `release:` consumes the preserved dist via `release --publish-only`) |
