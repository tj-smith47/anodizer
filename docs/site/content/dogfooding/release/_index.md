...
| `release.metadata` | ✅ Verified | `release.include_meta: true` attaches [v0.23.0 `metadata.json`](https://github.com/tj-smith47/anodizer/releases/download/v0.23.0/metadata.json) to every release. `artifacts.json` is deliberately *not* attached — it is the dist directory's local manifest, dropped from the uploadable set in v0.2.0 so `include_meta` ships exactly one file (see [metadata](/docs/general/metadata/#uploading-metadata-files)). The [v0.1.1 `artifacts.json`](https://github.com/tj-smith47/anodizer/releases/download/v0.1.1/artifacts.json) asset is the last one that predates that change |
| `release.name_template` / `tag_template` | ✅ Verified | Both shapes. Lockstep: [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) sets one `crates.tag_template: "v{{ Version }}"` that every crate shares, producing a single tag per release. Per-crate: [cfgd `.anodizer.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer.yaml) gives each workspace its own (`crd-v` / `core-v` / `v` / `operator-v` / `csi-v`), producing five independent tag families — see [cfgd's tags](https://github.com/tj-smith47/cfgd/tags) |
...
| `changelog.groups` | ✅ Verified | The "Features" and "Bug Fixes" groups, each with its `regexp` + `order`, render as headed sections in the [v0.23.0 release body](https://github.com/tj-smith47/anodizer/releases/tag/v0.23.0) |
...
| `changelog.use: git` | ✅ Verified | Lockstep. Live on every anodizer release — [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`use: git`) renders the grouped `## Changelog` in each [release body](https://github.com/tj-smith47/anodizer/releases/tag/v0.23.0). Dispatch: [`crates/stage-changelog/src/run.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-changelog/src/run.rs) |
| `changelog.use: github-native` | ✅ Verified | Single-crate. [brontes `.anodizer.yaml` at v0.2.0](https://github.com/tj-smith47/brontes/blob/v0.2.0/.anodizer.yaml) (`use: github-native`) rendered the [v0.1.0](https://github.com/tj-smith47/brontes/releases/tag/v0.1.0) and [v0.2.0](https://github.com/tj-smith47/brontes/releases/tag/v0.2.0) release bodies (dogfooded through v0.2.0; brontes moved to `use: git` for v0.2.1). Code path: [`crates/stage-changelog/src/github_native.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-changelog/src/github_native.rs) |
| `changelog.use: github` | ✅ Verified (tests) | [`crates/stage-changelog/src/run.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-changelog/src/run.rs) (`use: github` arm of the source dispatch), covered in [`crates/stage-changelog/src/tests.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-changelog/src/tests.rs); no live release selects it |
| `changelog.use: gitlab` / `gitea` | ✅ Verified (tests) | [`crates/stage-changelog/src/run.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-changelog/src/run.rs) (`gitlab` / `gitea` arms of the source dispatch), covered in [`crates/stage-changelog/src/tests.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-changelog/src/tests.rs); we dogfood on GitHub only |
...
These features shipped 2026-05-14 in response to the anodizer **v0.2.0 cascade
...
three-group publisher dispatch (Assets, Manager, Submitter) and a Submitter
gate that aborts the Submitter group when required Assets or Manager
publishers fail. Several behaviors have unit/integration test coverage today
(rows marked `✅ Verified (tests)` below); rows that need a live v0.2.x+ tag
to exercise the codepath stay `🤝 Help wanted`.

The rollback machinery evolved further on 2026-07-20: automatic in-process
rollback (`--rollback`, `on_failure: rollback`, and the `--rollback-only
--from-run=<id>` replay path) was **removed** in favor of a convergent
model — every publisher implements `reconcile()`, so a re-run of the
identical command safely skips whatever already published. Deliberate
withdrawal moved to `anodizer tag rollback`, which now also unwinds
previously-succeeded publishers via the same `rollback()` overrides the old
replay path drove. Rows below that document the removed surface are kept
for historical record — the run evidence they cite is real and still
demonstrates the failure mode that motivated the redesign — and are marked
...
| `--rollback=best-effort` | ⛔ Removed (2026-07-20) | The flag and the automatic rollback it triggered are gone; the replay executor relocated into `anodizer tag rollback`'s publisher-unwind step (still exercises `rollback()` overrides). Historical evidence — the best-effort rollback path fired live (as the then-default failure policy) in the failed v0.15.1 publish ([run 28809062839](https://github.com/tj-smith47/anodizer/actions/runs/28809062839)): `required failure(s) detected; invoking best-effort rollback` → `dispatching rollback for 12 target(s)` → `rollback complete — 7 rolled back, 0 failed, 3 skipped-no-scope`. See [Release Resilience — Recovering a poisoned tag](@/docs/advanced/release-resilience.md#recovering-a-poisoned-tag-with-tag-rollback) |
| `--rollback-only --from-run=<id>` replay | ⛔ Removed (2026-07-20) | The flag is gone; the same replay executor now runs as `anodizer tag rollback`'s publisher-unwind step, triggered by deliberate withdrawal rather than a CLI flag. Its live coverage moved with it: [`crates/cli/src/commands/tag/rollback/unwind.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/cli/src/commands/tag/rollback/unwind.rs) — both dist layouts discovered from the tag alone, dry-run mutating nothing, dispatch persisting `rollback.json`, and unreadable state refusing unless `--force` (`unwind::tests::*`); end-to-end wiring through the command (`commands::tag::rollback::tests::run_unwinds_recorded_publishers_before_deleting_tags`), plus the guard-runs-first ordering ([`crates/cli/src/commands/tag/rollback/tests.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/cli/src/commands/tag/rollback/tests.rs) `run_skips_the_unwind_when_the_guard_refuses`). The `--from-run` path-traversal guard survives on `promote --from-run`, the one remaining consumer (`crates/cli/tests/integration.rs::promote_from_run_rejects_path_traversal_at_binary_surface`) |
| `--fail-fast` | ✅ Verified (tests) | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) plus [release command wiring](https://github.com/tj-smith47/anodizer/blob/master/crates/cli/src/commands/release/mod.rs) (`fail_fast` opts); dispatcher coverage via `dispatch::tests::fail_fast_aborts_at_first_error` — pre-resilience-work flag, exercised in v0.1.x runs but no live v0.2.x release has tripped it; the default collect-then-bail mode it inverts is live-proven at [run 28809062839](https://github.com/tj-smith47/anodizer/actions/runs/28809062839) |
...
| Preflight rollback-scope checks | ✅ Verified (tests) | Still live post-2026-07-20, retargeted: the checked scope is the credential `anodizer tag rollback` will need if the release is ever withdrawn, rather than an in-release automatic rollback. [`crates/stage-publish/src/preflight/checkers.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-publish/src/preflight/checkers.rs) — warn / strict-block paths covered in [`crates/stage-publish/src/preflight/tests.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-publish/src/preflight/tests.rs) by `preflight_warns_on_missing_rollback_scope`, `preflight_blocks_on_missing_rollback_scope_when_strict`; the third arm (a required publisher's missing scope bailing the run under the old best-effort policy) went away with automatic rollback, and `config::tests::test_validate_on_failure_not_rollback_rejects_root_setting` now pins that `on_failure: rollback` is a hard config error. No live release has tripped the remaining checks |
...
| Snapcraft double-publish fix (`SnapcraftPublisher` unregistered unconditionally) | ✅ Verified (tests) | [`crates/stage-publish/src/registry.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-publish/src/registry.rs) — `registry::tests::snapcraft_unconditionally_unregistered_regardless_of_publish_flag` asserts the registry never re-registers `SnapcraftPublisher` alongside the real `SnapcraftPublishStage` ([`crates/stage-snapcraft/src/publish_stage/mod.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-snapcraft/src/publish_stage/mod.rs)) regardless of the `publish:` flag, preventing the v0.2.0 double-upload regression (commit `b3791cf`). Live behavior matches: the v0.15.5 publish ([run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907)) shows exactly one snap publish stage and a single `snapcraft` row in the run summary |
| Required-failure → non-zero exit gate | ✅ Verified | [`crates/stage-publish/src/lib.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-publish/src/lib.rs) (`gate_required_failures`) — short-circuit / snapshot / dry-run / rollback-failed / optional-failure / missing-report branches covered by unit tests in the same module; commit `1d9a13e`. Fired live in both directions: the failed v0.15.1 publish ([run 28809062839](https://github.com/tj-smith47/anodizer/actions/runs/28809062839)) exited non-zero on `1 required publisher(s) failed: gemfury` (job failed with exit code 1), while v0.15.5 ([run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907)) kept passing despite an *optional* snapcraft failure — the optional-failure branch live |
| `--strict` (fold with `preflight.strict`) | ✅ Verified (tests) | [`crates/core/src/context/mode.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/core/src/context/mode.rs) (`Context::preflight_is_strict`, the effective-strictness combiner) — either flag turns strict on, covered by `crates/cli/src/commands/release/tests.rs::strict_or_config_strict_promotes_preflight_to_strict`; promotion of indeterminate credential/permission probes (5xx / rate-limit / inconclusive) to blockers covered by [`crates/core/src/git/github_probe.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/core/src/git/github_probe.rs) `strict_promotes_indeterminate_arms_to_blocker`; mutex with `--allow-nondeterministic` covered by `crates/cli/tests/integration.rs::release_strict_conflicts_with_allow_nondeterministic` |
| Convergent re-run (per-publisher `reconcile()`) | ✅ Verified (tests) | [`crates/core/src/publisher.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/core/src/publisher.rs) (`ReconcileState`) + [`crates/stage-publish/src/dispatch.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-publish/src/dispatch.rs) — the reconcile arm's `Absent` / `Complete` / `Diverged` / `Unknown` branches, its dry-run and `simulate_failure` bypasses, and required-vs-optional `Diverged` handling are covered by the `reconcile*` unit tests in the same crate; per-publisher coverage for cargo, chocolatey, npm, pypi, winget, homebrew, homebrew-core, scoop, nix and krew sits beside each publisher. This replaces the former `--allow-rerun` refuse-guard: re-running is now the safe happy path, and `crates/cli/tests/integration.rs::test_release_allow_rerun_flag_is_rejected_as_unknown` pins that the removed flag hard-errors instead of silently no-op'ing |
...
| Real DELETE rollback for `cloudsmiths[]` | ✅ Verified (tests) | [`crates/stage-publish/src/cloudsmith/publisher.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-publish/src/cloudsmith/publisher.rs) — slug captured at upload time round-trips through `PublishEvidence.extra` so rollback can issue real `DELETE` against the Cloudsmith API; coverage in [`crates/stage-publish/src/cloudsmith/publisher_tests.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-publish/src/cloudsmith/publisher_tests.rs) via `cloudsmith_target_serde_roundtrip_with_slug`, `cloudsmith_target_decode_tolerates_missing_slug_field`, `cloudsmith_target_decode_tolerates_null_slug`, `cloudsmith_rollback_falls_back_to_warn_when_slug_missing`, `cloudsmith_rollback_warns_when_no_targets_recorded` (commit `8a79bf1`) |
| `RunSummary` dynamic-width status table | ✅ Verified | [`crates/stage-publish/src/run_summary.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-publish/src/run_summary.rs) (`status_table_rows`) — width adapts to the longest publisher name (capped at 40 chars with UTF-8-safe ellipsis truncation); covered by `status_table_rows_render_per_publisher_and_run_flags`, `status_table_rows_keep_long_names_untruncated_under_cap`, `status_table_rows_truncate_extremely_long_names` (commit `52c51da`). Rendered live at the end of both v0.15.5 ([run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907)) and the failed v0.15.1 run — 17-publisher status table with per-group/required/status columns |
| One-way-door burn probes (crates.io / Chocolatey / winget) | ✅ Verified (tests) | [`crates/stage-publish/src/cargo/already_published.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-publish/src/cargo/already_published.rs), [`crates/stage-publish/src/chocolatey/mod.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-publish/src/chocolatey/mod.rs), [`crates/stage-publish/src/post_publish/winget.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-publish/src/post_publish/winget.rs) — before rollback touches an irreversible registry, a probe checks whether the version is already published (crates.io index, Chocolatey package page scrape — the flat OData feed hides pending moderation — and the winget upstream PR search). Positive evidence refuses rollback; ambiguity stays clear. No live release has tripped a burn probe yet — in the one live failure so far ([run 28809062839](https://github.com/tj-smith47/anodizer/actions/runs/28809062839)) the Submitter gate stopped cargo/chocolatey/winget *before* they published, so there was no published one-way-door publish for a probe to check |
| `release.on_failure` policy (`rollback` \| `hold`) + auto-degrade past burned doors | ⛔ Removed (2026-07-20) | `on_failure: rollback` and the auto-degrade-to-`hold` logic are gone; `hold` is now the only accepted value, and `rollback` is a hard config-validation error. Historical evidence for the removed `rollback` path: it fired live in the failed v0.15.1 publish ([run 28809062839](https://github.com/tj-smith47/anodizer/actions/runs/28809062839)) — rollback executed in-process and the publish-only hold rule kept the pre-existing tag + bump commit in place (`publish-only run failed — holding the already-released tag and version-bump commit in place …`). `hold` itself remains live and is wired in [brontes `.anodizer.yaml`](https://github.com/tj-smith47/brontes/blob/master/.anodizer.yaml) (`release.on_failure: hold`). See [Release Resilience — `release.on_failure`](@/docs/advanced/release-resilience.md#release-on-failure) |
...
| Retry backoff attribution in the run summary | ✅ Verified (tests) | [`crates/core/src/retry/mod.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/core/src/retry/mod.rs) + [`crates/stage-publish/src/run_summary.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-publish/src/run_summary.rs) — total backoff wait is accounted run-wide and attributed per publisher/stage in the summary, so a slow release is explainable from the audit artifact. v0.15.5's snapcraft retries ([run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907)) accrued real backoff, but that run's `summary.json` (written by the pre-attribution v0.15.4 binary) carries no backoff fields — the attribution published after v0.15.5 and awaits the next release for live proof |
...
| Key | Status | Notes |
|---|---|---|
| `anodizer check determinism --runs=N` harness | ✅ Verified | [anodizer `release.yml`](https://github.com/tj-smith47/anodizer/blob/v0.23.0/.github/workflows/release.yml) (`determinism-check:` calls the reusable [`determinism.yml`](https://github.com/tj-smith47/anodizer/blob/v0.23.0/.github/workflows/determinism.yml) 4-shard matrix on every tag; the `release:` job consumes the preserved dist via `release --publish-only`) |
| `anodizer check config` (post-restructure) | 🤝 Help wanted | [`crates/cli/src/commands/check/config/mod.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/cli/src/commands/check/config/mod.rs) - post-restructure config validator; no release has exercised the new surface yet |
...
| `--inject-drift=archive\|sbom` test hook (`ANODIZE_TEST_HARNESS=1` gated) | ✅ Verified (tests) | [`crates/cli/src/determinism_harness/drift.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/cli/src/determinism_harness/drift.rs) (`inject_drift_byte`) + env-gate in [`crates/cli/src/commands/check/determinism/mod.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/cli/src/commands/check/determinism/mod.rs) — end-to-end drift detection covered by `crates/cli/tests/check_determinism.rs::inject_drift_archive_reports_drift_on_minimal_workspace` and the unit-level mutation injection point at `determinism_harness::drift::tests::inject_drift_byte_mutates_file_so_hash_differs` |
...
| SBOM byte-stability under `SOURCE_DATE_EPOCH` | ✅ Verified (tests) | [`crates/stage-sbom/src/builtin.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-sbom/src/builtin.rs) — CycloneDX output byte-stable for the same timestamp + honors / varies with SDE; coverage in [`crates/stage-sbom/src/tests.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-sbom/src/tests.rs) via `cyclonedx_output_byte_stable_for_same_timestamp`, `sbom_metadata_timestamp_honors_sde`, `sbom_metadata_timestamp_varies_with_sde` (commit `4a34d1a`) |

## Announcers

13 channels implemented. Two run on live releases of both anodizer and cfgd;
the other eleven have full test coverage but no live secrets configured.

| Key | Status | Notes |
|---|---|---|
| `announce.webhook` | ✅ Verified | Lockstep and per-crate. [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) posts a JSON payload (`project` / `tag` / `url`) with an `X-Anodizer-Source: release` header and `expected_status_codes: [200, 202]`; it fired at v0.15.5 ([run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907)) once the required publishers passed the `gate_on` check. cfgd wires the same endpoint from an `includes:` fragment, [`.anodizer/announce.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer/announce.yaml) |
| `announce.smtp` | ✅ Verified | Lockstep and per-crate. [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (Gmail SMTP, port 587, `encryption: starttls`) sent alongside the webhook at v0.15.5 ([run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907)); cfgd wires the same channel in [`.anodizer/announce.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer/announce.yaml) |
...
| Submission attribution (PR footers + generated-file headers) | ✅ Verified | The generated-file header is live in the tap: [Casks/anodizer.rb](https://github.com/tj-smith47/homebrew-tap/blob/master/Casks/anodizer.rb) (v0.23.0) opens with `# This file was generated by anodizer (https://github.com/tj-smith47/anodizer). DO NOT EDIT.`. [`crates/stage-publish/src/util/attribution.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-publish/src/util/attribution.rs); the `Automatically submitted by anodizer` PR footer reaches the next external-index submission PR |
...
The registry has served anodizer since 0.6.0 and now holds
[34 published versions](https://registry.modelcontextprotocol.io/v0/servers?search=anodizer)
through 0.23.0 — one per release, each pointing at that release's OCI image.
Every publish authenticates by exchanging the workflow's GitHub OIDC token; no
registry credential is stored.

| Key | Status | Notes |
|---|---|---|
| `mcp.name` | ✅ Verified | [`io.github.tj-smith47/anodizer`](https://registry.modelcontextprotocol.io/v0/servers?search=anodizer) is the live server name in the registry, set by [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) |
| `mcp.packages[]` | ✅ Verified | Each published version carries `registry_type: oci` + `identifier: ghcr.io/tj-smith47/anodizer` + `transport: stdio` — visible in the [registry entries](https://registry.modelcontextprotocol.io/v0/servers?search=anodizer), resolving to the image `dockers_v2:` pushed for that release |
| `mcp.auth.type: github-oidc` | ✅ Verified | The only auth path any of the 34 published versions used: [`crates/stage-publish/src/mcp/auth.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-publish/src/mcp/auth.rs) exchanges the Actions id-token, and [`publish-oidc.yml`](https://github.com/tj-smith47/anodizer/blob/master/.github/workflows/publish-oidc.yml) grants the `id-token: write` it needs |
...
| `mcp.repository` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`url: https://github.com/tj-smith47/anodizer`, `source: github`) renders into the `repository` block of every live [registry entry](https://registry.modelcontextprotocol.io/v0/servers?search=anodizer); manifest assembly at [`crates/stage-publish/src/mcp/manifest.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-publish/src/mcp/manifest.rs) |
