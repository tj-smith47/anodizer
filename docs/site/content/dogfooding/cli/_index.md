...
[anodizer's `release.yml`](https://github.com/tj-smith47/anodizer/blob/v0.12.3/.github/workflows/release.yml),
[`ci.yml`](https://github.com/tj-smith47/anodizer/blob/v0.12.3/.github/workflows/ci.yml),
...
|---|---|---|
| `release` | ✅ Verified | [anodizer `release.yml`](https://github.com/tj-smith47/anodizer/blob/v0.12.3/.github/workflows/release.yml) (`args: release --publish-only`) |
...
| `jsonschema` | ✅ Verified | [`docs.yml`](https://github.com/tj-smith47/anodizer/blob/v0.12.3/.github/workflows/docs.yml) regenerates [`schema.json`](https://github.com/tj-smith47/anodizer/blob/master/docs/site/static/schema.json) via `anodizer jsonschema` |
...
| `tag` | ✅ Verified | anodizer's prior releases (v0.2.0–v0.5.0) were auto-tagged from Conventional Commits; the tag is now cut by [`release.yml`](https://github.com/tj-smith47/anodizer/blob/v0.12.3/.github/workflows/release.yml)'s `workflow_run` tag job |
...
| `check determinism` | ✅ Verified | [anodizer `release.yml`](https://github.com/tj-smith47/anodizer/blob/v0.12.3/.github/workflows/release.yml) (`determinism: 'true'` per shard on the 3-OS matrix; `args: release --check determinism --preserve-dist` on prior releases) |
...
| Flag | Status | Notes |
|---|---|---|
| `--single-target` | ✅ Verified | [anodizer `ci.yml`](https://github.com/tj-smith47/anodizer/blob/v0.12.3/.github/workflows/ci.yml) (`args: release --snapshot --single-target --clean --dry-run`) |
| `tag --push` | ✅ Verified | [`release.yml`](https://github.com/tj-smith47/anodizer/blob/v0.12.3/.github/workflows/release.yml)'s `workflow_run` tag job (`args: tag --push --changelog`, pushed via `GITHUB_TOKEN`) cuts every release off `master`; tags `v0.12.0`–`v0.12.3` were all pushed this way (`gh release list`). Also covered by integration tests (bare-remote fixture asserts remote branch HEAD == tag target, no orphan) |
...
| `--publish-only` | ✅ Verified | [anodizer `release.yml`](https://github.com/tj-smith47/anodizer/blob/v0.12.3/.github/workflows/release.yml) (`args: release --publish-only`) |
...
| `--auto-snapshot` | ✅ Verified | [anodizer `ci.yml`](https://github.com/tj-smith47/anodizer/blob/v0.12.3/.github/workflows/ci.yml) (snapshot dry-run on master) |
...
| `tag --changelog` | ✅ Verified | [`release.yml`](https://github.com/tj-smith47/anodizer/blob/v0.12.3/.github/workflows/release.yml)'s tag job passes `args: tag --push --changelog` on every auto-tag; the [v0.12.3 release body](https://github.com/tj-smith47/anodizer/releases/tag/v0.12.3) carries the rendered `## Changelog` → `### Bug Fixes` groups. See [`crates/cli/src/commands/tag/mod.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/cli/src/commands/tag/mod.rs) (renders and stages changelogs atomically with the version-sync commit) |
