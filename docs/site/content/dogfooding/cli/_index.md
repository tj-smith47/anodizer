...
| `tag --push-tags-only` | ✅ Verified (historical) | anodizer's auto-tag job ran `args: tag --changelog --push-tags-only` through [v0.23.0](https://github.com/tj-smith47/anodizer/blob/v0.23.0/.github/workflows/release.yml); the deferred branch fast-forward it requires races any push to the release branch, so anodizer returned to the atomic `tag --push` and the flag has no current live consumer |
