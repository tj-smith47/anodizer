...
| `tag --push` | ⏳ Pending | Wired in [`release.yml`](https://github.com/tj-smith47/anodizer/blob/v0.12.3/.github/workflows/release.yml)'s `workflow_run` tag job (`args: tag --push`, pushed via `GITHUB_TOKEN`). Covered by integration tests (bare-remote fixture asserts remote branch HEAD == tag target, no orphan). Awaits the first release off `master` for live proof |
