...
| **Snap Store** | ⏳ Pending | [snapcraft.io/anodizer](https://snapcraft.io/anodizer) serves 0.9.1 (uploaded 2026-06-13) and [snapcraft.io/cfgd](https://snapcraft.io/cfgd) serves 0.3.5 — the newest revisions either project ever published. Both `snapcrafts:` blocks now carry `publish: false`: each tool execs host binaries that `strict` confinement blocks, and the Snap Store denied both `classic` requests. Uploads resume if a grant arrives |
