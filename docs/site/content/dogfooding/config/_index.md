...
| `nightly.*` | ✅ Verified | Both shapes. [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`tag_name: nightly`, `publish_release: true`, `keep_single_release: true` — one rolling release rather than an accumulating pile) driven by [`nightly.yml`](https://github.com/tj-smith47/anodizer/blob/master/.github/workflows/nightly.yml) on a `0 4 * * *` cron; [cfgd `.anodizer.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer.yaml) runs the per-crate equivalent |
