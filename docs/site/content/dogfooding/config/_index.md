...
| `env_files` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`env_files: [.env.release]` — non-secret release config loaded from a tracked file, overridable per environment); parser at [`crates/core/src/config/mod.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/core/src/config/mod.rs) |
| `variables` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`variables.repo_url` + `.description`). cfgd carried the same block through [v0.4.0](https://github.com/tj-smith47/cfgd/blob/v0.4.0/.anodizer.yaml) (dropped on master 2026-06-11) |
...
| `includes[].from_file` | ✅ Verified | [cfgd `.anodizer.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer.yaml) (`includes: - from_file: path: .anodizer/announce.yaml` — the announce matrix lives in [`.anodizer/announce.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer/announce.yaml) and merges as defaults beneath the root config; live since [v0.5.0](https://github.com/tj-smith47/cfgd/blob/v0.5.0/.anodizer.yaml)). Parser: [`crates/core/src/config/mod.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/core/src/config/mod.rs) (`IncludeSpec`) |
...
| `--auto-snapshot` | ✅ Verified | [anodizer `ci.yml`](https://github.com/tj-smith47/anodizer/blob/v0.23.0/.github/workflows/ci.yml) (snapshot build on every master push) |
| `nightly.*` | ✅ Verified | Both shapes. [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`publish_release: true`, `keep_single_release: true`, `skip_if_no_changes: true` — one rolling release rather than an accumulating pile, and no release at all on a quiet day) driven by [`nightly.yml`](https://github.com/tj-smith47/anodizer/blob/master/.github/workflows/nightly.yml) on a `0 4 * * *` cron; [cfgd `.anodizer.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer.yaml) runs the per-crate equivalent |
...
| `{{ .Var.* }}` | ✅ Verified (tests) | [`crates/core/src/template/vars.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/core/src/template/vars.rs) (`Var.*` binding from the `variables:` block). No live config interpolates `{{ Var.* }}` in a template string — anodizer declares the `variables:` block but references it only from docs |
| `{{ .PrefixedTag }}` | ✅ Verified | [`crates/core/src/context/populate.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/core/src/context/populate.rs) (`PrefixedTag` binding — the full tag including a monorepo or `tag_prefix` prefix, while `{{ .Tag }}` is the stripped form) |
| `{{ .Artifacts }}` | ✅ Verified (tests) | [`crates/core/src/context/populate.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/core/src/context/populate.rs) (the produced-artifact list, iterable as `{% for a in Artifacts %}`), covered in [`crates/core/src/context/tests.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/core/src/context/tests.rs). No dogfood config interpolates it — the `docker_manifests[].image_templates` use case it exists for is bypassed because `dockers_v2` pushes a multi-arch index directly |
...
| Key | Status | Notes |
|---|---|---|
| `monorepo.tag_prefix` | ✅ Verified | Per-crate only — a lockstep workspace cuts one `v{{ Version }}` tag for every crate, so the prefix has nothing to distinguish. cfgd cuts five distinct tag families ([cfgd `.anodizer.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer.yaml): `crd-v` / `core-v` / `v` / `operator-v` / `csi-v`), visible as [cfgd's tag list](https://github.com/tj-smith47/cfgd/tags) |
| `monorepo.dir` | ✅ Verified | Per-crate. [cfgd `.anodizer.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer.yaml) (`path: crates/cfgd-crd`, `crates/cfgd-core`, `crates/cfgd`, `crates/cfgd-operator`, `crates/cfgd-csi`) |
| `cargo_workspace` detection | ✅ Verified | Per-crate. [cfgd `.anodizer.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer.yaml) (5 `workspaces:` entries: cfgd-crd, cfgd-core, cfgd, cfgd-operator, cfgd-csi) |
| `depends_on` | ✅ Verified | Per-crate only. Independent workspaces need an explicit edge to order their publishes; a lockstep workspace derives the order from each `Cargo.toml` instead, which is why [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) declares none. [cfgd `.anodizer.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer.yaml) carries `depends_on: [cfgd-core]` on its downstream crates |

## Publisher resilience

| Key | Status | Notes |
|---|---|---|
| `publish.on_error` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`defaults.publish.on_error` runs a `cmd` per failed publisher before rollback; failure context arrives as `ANODIZER_PUBLISHER`/`ANODIZER_ERROR`/`ANODIZER_VERSION`/`ANODIZER_TAG`/`ANODIZER_GROUP`/`ANODIZER_REQUIRED`/`ANODIZER_ROLLED_BACK` env vars on the hook process — read these instead of interpolating untrusted error text into the shell string — with matching template vars for trusted values). Workspace-wide; per-crate entries append before defaults. Fired live in the failed v0.15.1 publish ([run 28809062839](https://github.com/tj-smith47/anodizer/actions/runs/28809062839)): `ran on-error hook: anodizer notify --raw "anodizer: publisher $ANODIZER_PUBLISHER failed …"` after the gemfury failure |
| `publish.on_rollback` | ✅ Verified (tests) | [`crates/core/src/config/publishers/mod.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/core/src/config/publishers/mod.rs) (`on_rollback:` hook list) + [`crates/stage-publish/src/failure_hooks.rs`](https://github.com/tj-smith47/anodizer/blob/master/crates/stage-publish/src/failure_hooks.rs) — runs a `cmd` per publisher whose published publish was rolled back (independent of `on_error`), with the rollback trigger reason exposed to the hook env. Neither dogfood config wires it yet |
| `defaults.publish.cargo.retain_on_rollback` | ⏳ Pending | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`retain_on_rollback: true` under `defaults.publish.cargo` — crates.io publishes are permanent; retain even if a downstream publisher rolls back). Not yet consulted live: in the one live rollback ([run 28809062839](https://github.com/tj-smith47/anodizer/actions/runs/28809062839)) cargo was Submitter-gated before publishing, so there was nothing to retain |
| `schemastore.retain_on_rollback` / `mcp.retain_on_rollback` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`retain_on_rollback: true` on the top-level `schemastore` and `mcp` keys). Fired live in the v0.15.1 rollback ([run 28809062839](https://github.com/tj-smith47/anodizer/actions/runs/28809062839)): `skipped rollback for 'mcp' — retain_on_rollback is set` and `skipped rollback for 'schemastore' — retain_on_rollback is set` |
...
| `tag.initial_version` | ✅ Verified | Per-crate. [cfgd `.anodizer.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer.yaml) (`initial_version: "0.5.0"` — the floor a newly added workspace starts from instead of 0.1.0) |
...
| `version_files` | ✅ Verified | Per-crate — the list is declared on the crate whose version it tracks, so each workspace rewrites its own files at its own cadence. [cfgd `.anodizer.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer.yaml) enrols `docs/installation.md`, `docs/bootstrap.md`, `docs/skill.md`, `chart/cfgd/Chart.yaml`; [cfgd v0.6.1 `docs/installation.md`](https://github.com/tj-smith47/cfgd/blob/v0.6.1/docs/installation.md) carries the tag-time-rewritten `0.6.1` download URLs, committed atomically with the version bump |
...
| `defaults.checksum.algorithm` | ✅ Verified | Lockstep. [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`algorithm: sha256`) → the combined [`anodizer_0.23.0_checksums.txt`](https://github.com/tj-smith47/anodizer/releases/download/v0.23.0/anodizer_0.23.0_checksums.txt) on every release |
...
| `changelog.footer` | ✅ Verified | [cfgd `.anodizer.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer.yaml) (a `footer: \|` block scalar whose body is `_Generated by anodizer._`) — anodizer itself now takes the derived release-body footer instead |
...
| `changelog.filters.exclude` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (excludes `docs`, `ci`, `chore`, `style`, `build`, `test`, `revert` and merge commits; each pattern carries the optional-scope group, e.g. `^docs(\(.*\))?!?:`, so scoped subjects are excluded too) |
...
| `changelog.paths` | ✅ Verified | [brontes `.anodizer.yaml`](https://github.com/tj-smith47/brontes/blob/master/.anodizer.yaml) (`paths: ["src/**", "tests/**", "examples/**", "Cargo.toml", "Cargo.lock"]` — only commits touching those paths enter the changelog; live on every brontes release) |
| `changelog.files.per_crate` | ✅ Verified | [cfgd `.anodizer.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer.yaml) (`per_crate: true`); live output: per-crate changelogs like [`crates/cfgd/CHANGELOG.md`](https://github.com/tj-smith47/cfgd/blob/master/crates/cfgd/CHANGELOG.md) and [`crates/cfgd-operator/CHANGELOG.md`](https://github.com/tj-smith47/cfgd/blob/master/crates/cfgd-operator/CHANGELOG.md), written by the v0.5.0 release-bump commit (`chore(release): bump cfgd-crd→0.3.5, cfgd-core→0.5.0, …`) |
...
| `source.files` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (globs: `crates/**/*.rs`, `Cargo.toml`, `Cargo.lock`, `LICENSE-MIT`, `LICENSE-APACHE`, `README.md`) |
...
| `checksum.name_template` | ✅ Verified | Per-crate. [cfgd `.anodizer.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer.yaml) (`checksum.name_template: "{{ ArtifactName }}.sha256"` on the `cfgd` workspace). anodizer leaves it unset and takes the combined-file default |
| `checksum.split` | ✅ Verified | Per-crate. [cfgd `.anodizer.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer.yaml) (`split: true` on the `cfgd` workspace — one `.sha256` sidecar per artifact instead of a combined file); live as the per-asset `.sha256` files on [cfgd v0.6.1](https://github.com/tj-smith47/cfgd/releases/tag/v0.6.1). anodizer runs the combined mode, so its releases carry `anodizer_<ver>_checksums.txt` and no sidecars |
...
| `snapcrafts[].name/title/summary/description` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml). Dogfooded through v0.9.1: [snapcraft.io/anodizer](https://snapcraft.io/anodizer) still serves the 0.9.1 revision uploaded 2026-06-13, the newest that was ever published |
| `snapcrafts[].base` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`base: core24`; dogfooded through the v0.9.1 store revision) |
| `snapcrafts[].grade/confinement` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`grade: stable`, `confinement: devmode`). anodizer needs `classic` — it execs the host's cargo/docker/gpg, which `strict` sandboxes — and the Snap Store has not granted the classic request, so the snap is parked on `devmode` |
| `snapcrafts[].publish` | ⏳ Pending | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) sets `publish: false`, so no current release uploads. The upload path itself is proven by the 0.9.1 store revision; re-enabling waits on the classic-confinement grant |
| `snapcrafts[].channel_templates` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`["latest/beta"]` — the Store only permits `devmode` uploads on beta/edge) |
| `snapcrafts[].apps` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (app with `home`, `network`, `network-bind` plugs; dogfooded through the v0.9.1 store revision) |
| `appimages[].desktop` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`assets/anodizer.desktop`); AppImages ship on every release — see the [v0.15.5 assets](https://github.com/tj-smith47/anodizer/releases/tag/v0.15.5) (`anodizer-0.15.5-{amd64,arm64}.AppImage`) built by [run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907) |
| `appimages[].icon` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`assets/logo.png`); embedded in the shipped [v0.15.5 AppImages](https://github.com/tj-smith47/anodizer/releases/tag/v0.15.5) |
| `appimages[].filename` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`"{{ ProjectName }}-{{ Version }}-{{ Arch }}.AppImage"`) — the shipped asset names `anodizer-0.15.5-amd64.AppImage` / `-arm64.AppImage` on the [v0.15.5 release](https://github.com/tj-smith47/anodizer/releases/tag/v0.15.5) are that template rendered |
| `appimages[].update_information` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`gh-releases-zsync\|...\|*.AppImage.zsync`); the resulting `.AppImage.zsync` delta-update files ship as release assets — [v0.15.5](https://github.com/tj-smith47/anodizer/releases/tag/v0.15.5) includes `anodizer-0.15.5-{amd64,arm64}.AppImage.zsync` |
...
| `publish.cargo.retain_on_rollback` | ⏳ Pending | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`retain_on_rollback: true` in defaults.publish.cargo); also wired in [brontes `.anodizer.yaml`](https://github.com/tj-smith47/brontes/blob/master/.anodizer.yaml). A live rollback has happened ([run 28809062839](https://github.com/tj-smith47/anodizer/actions/runs/28809062839)) but cargo was Submitter-gated before publishing, so this key was never consulted |
| `publish.cargo.index_timeout` | ✅ Verified | [cfgd `.anodizer.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer.yaml) (`defaults.publish.cargo.index_timeout: 600` — inherited by all 4 crates) · [brontes `.anodizer.yaml`](https://github.com/tj-smith47/brontes/blob/master/.anodizer.yaml) (same 600s setting; absorbs sporadic crates.io 503s) |
| `publish.cargo.locked` / `all_features` | ✅ Verified | [brontes `.anodizer.yaml`](https://github.com/tj-smith47/brontes/blob/master/.anodizer.yaml) (`publish.cargo: {locked: true, all_features: true}` — pins the CI-resolved `Cargo.lock` for the verify build and matches the docs.rs all-features render); live on [crates.io/crates/brontes](https://crates.io/crates/brontes) 0.3.0 |
| `pypis[].name` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`name: anodizer`) → [pypi.org/project/anodizer](https://pypi.org/project/anodizer/) at 0.23.0, `pip install anodizer` |
| `pypis[].auth: oidc` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`auth: oidc` — PyPI Trusted Publishing; the workflow's OIDC id-token is exchanged for a short-lived upload token, so no API token is stored). Runs from the github-hosted [`publish-oidc.yml`](https://github.com/tj-smith47/anodizer/blob/master/.github/workflows/publish-oidc.yml), the workflow PyPI's Trusted Publisher config names |
| `pypis[].requires_python` / `project_urls` / `classifiers` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`>=3.8`, Documentation + Repository links, three classifiers) — all four render on the live [project page](https://pypi.org/project/anodizer/) |
| per-platform wheel set | ✅ Verified | One release produces 9 wheels from the same binaries the archives ship — macOS x86_64 / arm64 / universal2, manylinux + musllinux for x86_64 and aarch64, Windows amd64 / arm64 — all listed under [0.23.0's downloads](https://pypi.org/project/anodizer/#files). The platform tags are derived from the configured targets, not restated in config |
| `publish.krew` | ✅ Verified | Per-crate. cfgd's CLI crate declares `publish.krew` in [cfgd `.anodizer.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer.yaml), describing the `kubectl-cfgd` plugin rather than the CLI; the upstream index serves [`plugins/cfgd.yaml`](https://github.com/kubernetes-sigs/krew-index/blob/master/plugins/cfgd.yaml) at v0.6.1 |
| `homebrew_cores[]` | 🤝 Help wanted | Implemented, but homebrew-core's notability and versioning rules exclude all three of our projects, so no dogfood config declares it. See [`homebrew_core:` docs](../../../docs/publish/homebrew-core/) |
| `aur_sources[]` | ✅ Verified | Lockstep. [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`aur_sources:` renders a cargo build-from-source PKGBUILD, distinct from the prebuilt-binary `aur:` package) → [aur.archlinux.org/packages/anodizer](https://aur.archlinux.org/packages/anodizer) at 0.23.0-1, published alongside [`anodizer-bin`](https://aur.archlinux.org/packages/anodizer-bin) from the same release. See [`aur_sources:` docs](../../../docs/publish/aur-sources/) |
| `install_scripts` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) supplies only the banner metadata and `binaries: [anodizer]`; the os/arch→asset table, the `uname` detection arms, the supported-platform list, the checksums filename, the tag prefix, and the repo all derive from the configured targets. Live as the [`install.sh`](https://github.com/tj-smith47/anodizer/releases/download/v0.23.0/install.sh) asset on every release |
| `before_publish.hooks` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) — two `run_once` gates run after build/sign/checksum and before any one-way-door publisher dispatches: every checksum file must exist and be non-empty, and every release archive in each configured format must be a non-empty file. A non-zero exit aborts before crates.io, PyPI, or GitHub receive anything. See [`before_publish:` docs](../../../docs/publish/before-publish/) |
...
| `publish.aur.private_key` | ✅ Verified | [anodizer `release.yml`](https://github.com/tj-smith47/anodizer/blob/v0.23.0/.github/workflows/release.yml) (`AUR_SSH_KEY` secret) |
...
| Key | Status | Notes |
|---|---|---|
| `blobs[].provider` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`provider: s3`); live every release — v0.15.5 ([run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907)) logged `uploaded 47 object(s), skipped 0 (identical) → s3://…/v0.15.5` against the cluster-internal MinIO |
| `blobs[].bucket` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`anodizer-releases`); the live run summary records objects at `s3://anodizer-releases/v0.15.5/…` ([run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907)) |
| `blobs[].endpoint` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`"{{ Env.MINIO_ENDPOINT }}"` → `http://minio.jarvispro.svc.cluster.local:9003`); the live 47-object upload rides this endpoint ([run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907)) |
| `blobs[].region` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`region: us-east-1` — compatibility placeholder; MinIO ignores region); part of the live-exercised blobs entry ([run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907)) |
| `blobs[].s3_force_path_style` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`s3_force_path_style: true` — required for MinIO path-style addressing; without it the live upload cannot address the bucket at all). Live at [run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907) |
| `blobs[].disable_ssl` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`disable_ssl: true` — the in-cluster MinIO endpoint is plain http; without it the S3 client rejects the non-https endpoint). Live at [run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907) |
| `blobs[].directory` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`directory: "{{ Tag }}"`); rendered live as the `…/v0.15.5/` prefix on every uploaded object ([run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907)) |
...
| `dockerhub[].username` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`username: tsmthtj`); the description-sync publisher runs live against [hub.docker.com/r/tsmthtj/anodizer](https://hub.docker.com/r/tsmthtj/anodizer) — summary `dockerhub  Assets  optional  succeeded` at v0.15.5 ([run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907)) |
| `dockerhub[].description` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`description:` + `full_description.from_file: README.md`); at v0.15.5 the publisher read the remote repo and logged `skipped dockerhub PATCH … description / full_description match remote` — the synced text is live on [hub.docker.com/r/tsmthtj/anodizer](https://hub.docker.com/r/tsmthtj/anodizer) |
...
| `npms[].scope` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`scope: "@tj-smith47"`); the per-platform binary packages publish live under that scope (`@tj-smith47/anodizer-{darwin,linux,win32}-*` on [npm](https://www.npmjs.com/package/anodizer)) |
| `npms[].metapackage/bin/mode` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`metapackage: anodizer`, `bin: anodizer`, `mode: optional-deps` — biome/git-cliff pattern); the live [`anodizer`](https://www.npmjs.com/package/anodizer) metapackage resolves the matching binary via 8 `optionalDependencies` (`npm view anodizer optionalDependencies`) |
...
| `mcp.retain_on_rollback` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`retain_on_rollback: true`); fired live in the v0.15.1 rollback ([run 28809062839](https://github.com/tj-smith47/anodizer/actions/runs/28809062839)): `skipped rollback for 'mcp' — retain_on_rollback is set` |
...
| `schemastore.retain_on_rollback` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`retain_on_rollback: true`); fired live in the v0.15.1 rollback ([run 28809062839](https://github.com/tj-smith47/anodizer/actions/runs/28809062839)): `skipped rollback for 'schemastore' — retain_on_rollback is set` |

## Post-release

| Key | Status | Notes |
|---|---|---|
| `verify_release.enabled` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`enabled: true`); the stage ran live at v0.15.5 ([run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907)): `Verifying release … all post-publish checks passed`, summary `verify-release  passed`, and `verify_release.passed: true` in the run's `summary.json` |
| `verify_release.assert_assets` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`assert_assets: true` — every produced artifact must appear as an uploaded release asset); part of the live-passing verify at v0.15.5 ([run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907), `all post-publish checks passed`) |
| `verify_release.install_smoke` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`install_smoke: {}` — the empty block is `Some`, which enables the check: installs each nfpm package in a container and runs `<bin> --version`). Ran live at v0.15.5 ([run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907)): Docker was available on the publish runner and no `skipped install smoke-test` warning fired, so the smoke jobs ran within the passing verify. Also configured in [cfgd `.anodizer.yaml`](https://github.com/tj-smith47/cfgd/blob/master/.anodizer.yaml) |
| `verify_release.glibc_ceiling` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`glibc_ceiling: "2.36"` — fails if any `.deb` requires glibc > 2.36); part of the live-passing verify at v0.15.5 ([run 28882554907](https://github.com/tj-smith47/anodizer/actions/runs/28882554907)) |
| `attestations.enabled` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (`enabled: true`); the [v0.23.0 release](https://github.com/tj-smith47/anodizer/releases/tag/v0.23.0) ships the produced [`attestation-subjects.json`](https://github.com/tj-smith47/anodizer/releases/download/v0.23.0/attestation-subjects.json) asset |
| `attestations.mode` | ✅ Verified | Both modes live. `mode: subjects`: [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml) (writes `dist/attestation-subjects.json`; anodizer-action feeds it to `actions/attest-build-provenance`) — the 19 KB [`attestation-subjects.json`](https://github.com/tj-smith47/anodizer/releases/download/v0.23.0/attestation-subjects.json) attached to v0.23.0. `mode: emit`: [brontes `.anodizer.yaml`](https://github.com/tj-smith47/brontes/blob/master/.anodizer.yaml) (self-contained in-toto SLSA-provenance v1 statement) — the live [`attestation.intoto.jsonl`](https://github.com/tj-smith47/brontes/releases/download/v0.3.0/attestation.intoto.jsonl) attached to brontes v0.3.0 |
| `attestations.artifacts` | ✅ Verified | [brontes `.anodizer.yaml`](https://github.com/tj-smith47/brontes/blob/master/.anodizer.yaml) (`artifacts: [source, sbom]` — the [v0.3.0 `attestation.intoto.jsonl`](https://github.com/tj-smith47/brontes/releases/download/v0.3.0/attestation.intoto.jsonl) covers the source tarball and its SBOM). anodizer leaves it unset (defaults to all artifact kinds under `mode: subjects`) |
...
| `dmgs[]` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml); live [`anodizer_amd64.dmg`](https://github.com/tj-smith47/anodizer/releases/download/v0.23.0/anodizer_amd64.dmg) |
| `pkgs[]` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml); live [`anodizer_amd64.pkg`](https://github.com/tj-smith47/anodizer/releases/download/v0.23.0/anodizer_amd64.pkg) |
| `msis[]` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml); live [`anodizer_amd64.msi`](https://github.com/tj-smith47/anodizer/releases/download/v0.23.0/anodizer_amd64.msi) |
| `nsis[]` | ✅ Verified | [anodizer `.anodizer.yaml`](https://github.com/tj-smith47/anodizer/blob/master/.anodizer.yaml); live [`anodizer_x64-setup.exe`](https://github.com/tj-smith47/anodizer/releases/download/v0.23.0/anodizer_x64-setup.exe) |
